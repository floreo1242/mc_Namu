...
data class PlayerQuest(
    ...
    private fun getIncrementCount(playerQuestObjective: PlayerQuestObjective, inventoryMap: Map<ItemStack, Int>): Int {
        return if (playerQuestObjective.action == QuestAction.FARMING_ITEM) {
            var count = inventoryMap
                    .filter {
                        val customItem = it.key.getNBTTagCompound(CustomItem::class.java) ?: return@filter false

                        customItem.code == playerQuestObjective.target
                    }
                    .map { it.value }
                    .sum()
    ...
    fun takeQuestItems(player: Player, quest: Quest) {
        quest.questObjectiveList
                ...
                .forEach { questObjective ->
                    player.inventory.takeItem(CustomItemRepository.getCustomItem(questObjective.target)!!.toItemStack(), questObjective.amount)
                }
    }
    ...
    fun receiveRewards(player: Player, quest: Quest) {
        ...
        PlayerExpGetEvent(player)
                .run {
                    Bukkit.getPluginManager().callEvent(this@run)
                    if (!isCancelled) {
                        player.playerLevel.apply { exp += quest.reward.exp }.edit()
                    }
                }
