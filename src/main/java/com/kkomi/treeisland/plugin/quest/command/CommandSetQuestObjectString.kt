...
class CommandSetQuestObjectString(usage: String, description: String, argumentsLength: Int) : QuestCommandComponent(usage, description, argumentsLength) {
    ...
    override fun onCommand(commandSender: CommandSender, label: String, command: Command, componentLabel: String, args: ArgumentList, quest: Quest): Boolean {
        val stringObject = args.join(" ")
        quest.stringObject = stringObject
        QuestRepository.editQuest(quest)
        commandSender.sendInfoMessage(QuestMessage.QUEST_SET_OBJECT_STRING)
        return true
    }
