...
class CommandSetQuestDisposeMessage(usage: String, description: String, argumentsLength: Int) : QuestCommandComponent(usage, description, argumentsLength) {
    ...
    override fun onCommand(commandSender: CommandSender, label: String, command: Command, componentLabel: String, args: ArgumentList, quest: Quest): Boolean {
        val disposeMessage = args.join(" ")
        quest.disposeMessage = disposeMessage
        QuestRepository.editQuest(quest)
        commandSender.sendInfoMessage(QuestMessage.QUEST_SET_DISPOSE_MESSAGE)
        return true
    }
