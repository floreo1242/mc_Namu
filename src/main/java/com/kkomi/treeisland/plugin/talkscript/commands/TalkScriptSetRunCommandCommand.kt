...
class TalkScriptSetRunCommandCommand(usage: String, description: String, argumentsLength: Int) : TalkScriptCommandComponent(usage, description, argumentsLength) {
    override fun onCommand(sender: CommandSender, label: String, command: Command, componentLabel: String, args: ArgumentList, talkScript: TalkScript): Boolean {
        ...
        talkScript.runCommand = args.join(" ")
        TalkScriptRepository.editTalkScript(talkScript)
        sender.sendInfoMessage(TalkScriptMessage.SET_RUN_COMMAND)
        return true
    }
}